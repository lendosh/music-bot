...
export class StartCommand extends Command {
  ...
  handle(): void {
    ...
    this.bot.on(message('audio'), async (ctx) => {
      ...
      ctx.session.media = {
        title: 'local_file.mp3',
        path: `${ctx.session.userFolderPath}/local_file.mp3`,
      };
    ...
    this.bot.on(message('text'), async (ctx) => {
      ...
      if (isHostLink(urlData, 'youtu')) {
        ...
        ctx.session.media = {
          title: 'youtube_audio',
          path: `${ctx.session?.userFolderPath}/${MESSAGE_TEXT.PERFORMER}_youtube.mp3`,
    ...
    const replyToChanePinch = async (ctx: Context) => {
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          ...
          [Markup.button.callback('x0.8', 'x0.8'), Markup.button.callback('x0.9', 'x0.9')],
    ...
    const changePitch = async (pitch: string, ctx: IBotContext) => {
      console.log(1)
      const messageInfo = await ctx.sendMessage(PROCESS_STATUS.START);
    ...
    const downloadYTAudioAndSendToUser = async (
      ...
      ytdl(mediaUrl, {
        ...
        requestOptions: {
          headers: {
            cookie: 'x-youtube-identity-token:QUFFLUhqa2tYQTk5XzVBaXczeTRVNEIySFdUdnlBU2NmQXw=;'
          }
        }
      })
        ...
        .on('finish', async () => {
          await ctx.replyWithAudio({source: ctx!.session!.media!.path!}, {
            ...
            performer: MESSAGE_TEXT.PERFORMER,
            caption: MESSAGE_TEXT.CAPTION,
            duration: ctx.session?.media?.duration ? parseInt(ctx.session?.media?.duration) : undefined
          });
          await ctx.telegram.editMessageText(
            messageInfo.chat.id,
            messageInfo.message_id,
